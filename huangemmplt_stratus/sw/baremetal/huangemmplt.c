...
#define SLD_HUANGEMMPLT 0x02a
#define DEV_NAME "sld,huangemmplt_stratus"
...
int main(int argc, char * argv[])
{
	...
	// 	in_words_adj = huangemmplt_len*huangemmplt_vec;
	// 	out_words_adj = huangemmplt_vec;
	// } else {
	// 	in_words_adj = round_up(huangemmplt_len*huangemmplt_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// 	out_words_adj = round_up(huangemmplt_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// }


	// in_len = in_words_adj * (huangemmplt_n);
	// out_len = out_words_adj * (huangemmplt_n);
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_HUANGEMMPLT, DEV_NAME);
	if (ndev == 0) {
		printf("huangemmplt not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, HUANGEMMPLT_ROWS_REG, rows);
			iowrite32(dev, HUANGEMMPLT_COLS_REG, cols);
			iowrite32(dev, HUANGEMMPLT_LOADED_COLS_REG, loaded_cols);
